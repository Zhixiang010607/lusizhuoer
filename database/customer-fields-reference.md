...
- `created_by_account_id`：实际执行建档的登录账号。老师可以凭“该字段就是本人账号”立即查看自己建立的客户；门店账号建立时该字段保存门店账号，不会直接绑定任何老师。老师也会因有效工单的 `teacher_id` 经服务端归属本人，而取得同一客户的主页与全部历史工单只读权限。

历史客户在迁移 057 前没有记录创建账号，因此该字段保持 `NULL`，不能根据姓名、门店或时间猜测老师归属。老师仍可通过已通过工单上服务端验证的本人 `teacher_id` 关系查看这些历史客户。
