...
- 办理业务的客户下拉首批最多 100 位；姓名＋生日查询在服务端按当前门店精确执行，不为搜索预载整店客户。
- 客户主页的充值、核销历史分别使用 `(submitted_at,id)` 游标分批读取；审核列表首批最多 100 条，完整工单详情只读取 1 条。
- 客户主页照片请求与基础资料／历史请求并行；私有签名地址仍须在每次返回前重新鉴权，浏览器不得把照片或签名地址写入持久缓存。
- 老师工作台只按当前登录老师的 `teacher_id` 使用 `(submitted_at,id)` 游标读取本人充值与核销，复用迁移 025 的 `idx_recharge_teacher_cursor` 和 `idx_verification_teacher_cursor`；列表不加载任何客户照片，只有老师明确选择门店和客户进入办理流程时才签发该客户的短时照片地址。
