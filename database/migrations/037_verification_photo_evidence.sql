...
COMMENT ON TABLE public.verification_photos IS
  'Migration 037: one immutable face photo plus three submitter-managed supplemental slots per verification order.';
COMMENT ON TABLE public.verification_photo_drafts IS
  'Migration 037: short-lived face-verification photos awaiting atomic attachment to a verification order.';
COMMENT ON TABLE public.verification_photo_events IS
  'Migration 037: append-only audit of face binding, supplemental upload/replace, and original-photo viewing.';
COMMENT ON FUNCTION public.enforce_verification_photo_write() IS
  'Migration 037: face photo immutable; supplemental photos writable only by the order submitter before submitted_at + 24 hours.';
