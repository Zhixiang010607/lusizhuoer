-- Migration 040: remove the PL/pgSQL output-column ambiguity in photo commits.
-- Run once after migration 039. No table or data changes are required.
BEGIN;
...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT verification_photos_verification_id_photo_slot_key DO UPDATE
...
COMMENT ON FUNCTION public.commit_verification_photo_upload(
  VARCHAR, BIGINT, BIGINT, INTEGER, INTEGER, INTEGER, CHAR
) IS 'Migration 040: atomically commit a verified upload without output-column conflict ambiguity.';
