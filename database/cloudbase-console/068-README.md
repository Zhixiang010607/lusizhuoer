...
3. 部署 `customerRating v1` 前，为该函数配置至少 32 字节的 `CUSTOMER_RATING_SIGNING_KEY`，并把实际发布后的 `rating.html` 完整 HTTPS 地址填入必需的 `CUSTOMER_RATING_BASE_URL`；两者均不得放进网页、小程序、二维码、仓库或日志。CloudBase 还必须开启匿名登录、给匿名角色增加云函数网关权限，并对 `customerRating` 使用 `"auth != null"` 调用规则；数据库权限仍保持完全关闭。
