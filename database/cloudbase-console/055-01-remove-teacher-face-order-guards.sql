-- CloudBase migration 055. Run the whole file once after migration 054.
-- Fixes recharge/refund/verification/experience orders for no-photo teachers.
BEGIN;
...
AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.stores WHERE id = NEW.store_id AND store_status = 'ACTIVE') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.products WHERE id = NEW.product_id AND product_status = 'ACTIVE') THEN
...
COMMENT ON FUNCTION public.assert_active_order_master_data() IS
  'Active teacher/account required for new orders; teacher face enrollment is not required.';
COMMENT ON FUNCTION public.lock_active_verification_subjects(BIGINT, BIGINT, BIGINT, BIGINT, BIGINT) IS
  'Locks active subjects and customer retained photo without a teacher face dependency.';

COMMIT;

SELECT requirement, status
FROM (VALUES
  ('order guard has no teacher face dependency', CASE WHEN pg_get_functiondef('public.assert_active_order_master_data()'::regprocedure) !~* 'face_enrollment_status|face_person_id|unbound-face' THEN 'READY' ELSE 'STALE' END),
  ('verification lock has no teacher face dependency', CASE WHEN pg_get_functiondef('public.lock_active_verification_subjects(bigint,bigint,bigint,bigint,bigint)'::regprocedure) !~* 'face_enrollment_status|face_person_id|face enrollment' THEN 'READY' ELSE 'STALE' END),
  ('customer retained photo remains required', CASE WHEN pg_get_functiondef('public.lock_active_verification_subjects(bigint,bigint,bigint,bigint,bigint)'::regprocedure) ~* 'customer retained profile photo is required' THEN 'READY' ELSE 'MISSING' END)
) AS verification(requirement, status);
