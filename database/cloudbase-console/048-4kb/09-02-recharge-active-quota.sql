...
RETURNS TABLE(
  recharge_id BIGINT, created_now BOOLEAN, quota_id BIGINT, quota_month DATE, available_before_count INTEGER,
...
DECLARE
  ...
  m DATE := public.teacher_experience_quota_month();
  n TEXT;
  b INTEGER;
BEGIN
  ...
  SELECT * INTO r FROM public.teacher_experience_quota_recharges WHERE idempotency_key = p_idempotency_key;
  IF FOUND THEN
    IF r.teacher_id <> p_teacher_id OR r.product_id <> p_product_id
       OR r.unit_count <> p_unit_count OR r.note <> n
       OR r.recharged_by_account_id <> p_actor_account_id THEN
      RAISE EXCEPTION 'idempotency key belongs to a different teacher experience recharge' USING ERRCODE = '23505';
    END IF;
    SELECT * INTO q FROM public.teacher_product_experience_quotas WHERE id = r.quota_id;
    RETURN QUERY SELECT r.id, FALSE, r.quota_id, r.quota_month,
      r.available_before_count, r.available_after_count, q.monthly_allowance,
      q.used_count, q.manual_recharge_count, r.created_at;
    RETURN;
  END IF;
  PERFORM public.assert_active_teacher_experience_subjects(p_teacher_id, p_product_id);
  SELECT * INTO q FROM public.teacher_product_experience_quotas
   WHERE teacher_id = p_teacher_id AND product_id = p_product_id
     AND quota_status = 'ACTIVE' FOR UPDATE;
  IF NOT FOUND THEN
  ...
  UPDATE public.teacher_product_experience_quotas
   ...
   WHERE id = q.id AND quota_status = 'ACTIVE'
   RETURNING * INTO q;
  INSERT INTO public.teacher_experience_quota_recharges
    (quota_id, teacher_id, product_id, quota_month, unit_count,
     available_before_count, available_after_count, note, idempotency_key, recharged_by_account_id)
  VALUES
    (q.id, p_teacher_id, p_product_id, q.quota_month, p_unit_count, b,
     q.available_count, n, p_idempotency_key, p_actor_account_id)
  RETURNING * INTO r;
  RETURN QUERY SELECT r.id, TRUE, q.id, q.quota_month, b, q.available_count,
    q.monthly_allowance, q.used_count, q.manual_recharge_count, r.created_at;
END;
$$;
COMMIT;
