...
3. 继续执行 066；旧版 066 曾建立第三张设备注册表时先运行 `066-02-retire-legacy-device-registry.sql`。随后在低峰期执行 067，再上传 `faceRecognition-v110.zip` 并用 `health` 确认 `version=v110`；
...
4. 按 [`067-README.md`](067-README.md) 在低峰期建立大数据充值／退费查询索引，然后上传 `faceRecognition-v110.zip` 并确认 `health version=v110`；
...
4. 上传 `faceRecognition-v110.zip`，用 `health` 确认版本后，分别验收“今天”和“全部时间”的总部退费查询及翻页。
