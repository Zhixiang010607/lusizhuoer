...
assert.match(html, /styles\.css\?v=0\.15\.45/, "customer page should bust the stylesheet cache");
...
assert.match(ui, /PRODUCT_PURCHASE:\s*\{ hasMore:false/, "product purchase records must paginate independently");
...
assert.match(cloud, /\["RECHARGE", "REFUND", "VERIFICATION", "EXPERIENCE", "PRODUCT_PURCHASE"\]/, "customer history API must paginate all five histories independently");
assert.match(cloud, /r\.recharge_type = '\$\{refundOnly \? "REFUND" : "NEW"\}'/, "recharge and refund rows must be split by the server query");
