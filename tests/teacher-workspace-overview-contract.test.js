...
assert.match(ui, /function renderProfile[\s\S]{0,650}老师姓名[\s\S]{0,220}老师短编号/, "profile must keep the two useful teacher fields");
assert.doesNotMatch(ui, /loginIdentity|登录手机号|登录账号/, "teacher home must not repeat the login identity");
