...
test("summary values keep one readable size and overflow only inside the table", () => {
  ...
  assert.match(wxss, /grid-template-columns:\s*minmax\(180rpx,\s*1\.25fr\)\s*repeat\(4,\s*minmax\(195rpx,\s*1fr\)\)/);
  assert.match(wxss, /\.summary-metric\s*\{[^}]*display:\s*flex;/s);
  assert.match(wxss, /\.summary-metric\s*\{[^}]*min-width:\s*0;/s);
  assert.match(wxss, /\.metric-number\s*\{[^}]*font-size:\s*21rpx;/s);
  assert.match(wxss, /\.metric-unit\s*\{[^}]*font-size:\s*21rpx;/s);
