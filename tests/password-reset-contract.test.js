...
assert.doesNotMatch(loginHtml, /login-brand-panel|login-brand-copy|login-feature-list|login-security-note|demo-accounts|demoHint/,
  "login markup must not restore explanatory marketing or security copy");
assert.doesNotMatch(login, /demoHint/, "login runtime must not write hidden explanatory demo copy");
...
assert.ok(loginHtml.includes("cloudbase-phone-auth.js?v=0.20.0"), "login must load the UID-only phone-auth client");
