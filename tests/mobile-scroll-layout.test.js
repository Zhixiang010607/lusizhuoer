...
for (const file of ["customer-detail.html", "customer-query.html", "recharge-query.html", "verification-query.html"]) {
  ...
  assert(html.includes('styles.css?v=0.15.43'), `${file} must use the current stylesheet cache key`);
...
for (const file of fs.readdirSync(root).filter((name) => name.endsWith(".html"))) {
  ...
  assert(html.includes('styles.css?v=0.15.43'), `${file} must use the desktop-scroll stylesheet cache key`);
}
