...
test("HQ and store mini-program navigation exposes role-appropriate database searches", () => {
  ...
  for (const label of ["客户查询", "充值查询", "核销查询", "产品查询", "运营核心指标查询", "活跃预警", "余次预警", "评价分析"]) assert.match(homeWxml, new RegExp(label));
  ...
  assert.doesNotMatch(generalQuery, /活跃预警|余次预警|评价分析/,
  ...
  assert.match(coreMetrics, /运营核心指标查询[\s\S]*活跃预警[\s\S]*余次预警[\s\S]*评价分析/,
    "the separate operational-metrics dropdown must contain all three indicators");
  ...
  assert.match(homeJs, /pages\/rating-analysis\/index/);
...
test("customer query keeps the web filter dimensions, role scope, details, and direct page jump", () => {
  ...
  assert.doesNotMatch(wxml, /状态操作|bindtap="(?:toggleCustomerStatus|archiveCustomer|restoreCustomer)"/);
