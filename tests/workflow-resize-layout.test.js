...
for (const page of workflowPages) {
  ...
  assert.match(html, /styles\.css\?v=0\.15\.43/, `${page} should load the current responsive stylesheet`);
  assert.match(html, /store-business\.js\?v=0\.14\.52/, `${page} should load the current workflow script`);
}
