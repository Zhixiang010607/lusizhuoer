...
const workflowPages = ["recharge-create.html", "refund-create.html", "verification-create.html", "verification-experience.html"];
...
for (const page of workflowPages) {
  ...
  assert.match(html, /store-business\.js\?v=0\.14\.60/, `${page} should load the current workflow script`);
}
...
assert.match(script, /setupWorkflowResize\(\);\s*if \(legacyTeacherMode\)/, "workflow resizing should initialize before role-specific business setup");
