...
assert.match(detailHtml, /business-detail\.js\?v=0\.16\.7/);
...
assert.match(queryHtml, /query\.js\?v=0\.15\.7/);
...
assert.doesNotMatch(reviewHtml, /id="reviewType"|作废充值/);
assert.match(reviewHtml, /value="approved">审核通过<\/option>/);
assert.match(reviewHtml, /review\.js\?v=0\.17\.8/);
...
assert.match(accountCloud, /const FUNCTION_VERSION = "v42"/);
assert.match(accountCloud, /const ORDER_VOID_APPLICATIONS_ENABLED = false/);
assert.match(accountCloud, /applicationType: "NEW"/);
