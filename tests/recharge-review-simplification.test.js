...
assert.match(detailHtml, /business-detail\.js\?v=0\.16\.8/);
...
assert.match(queryHtml, /query\.js\?v=0\.15\.8/);
...
assert.match(reviewHtml, /id="reviewType"[\s\S]*value="NEW">充值申请<[\s\S]*value="REFUND">退费申请</);
assert.doesNotMatch(reviewHtml, /作废充值/);
assert.match(reviewHtml, /value="approved">审核通过<\/option>/);
assert.match(reviewHtml, /review\.js\?v=0\.17\.9/);
assert.match(reviewUi, /return value === "all" \? "" : value;/);
...
assert.match(accountCloud, /const FUNCTION_VERSION = "v43"/);
