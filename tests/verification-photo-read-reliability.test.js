...
assert.equal(photoPackage.version, "4.0.0", "reliable read layer has a distinguishable deployment package version");
...
async function healthIdentifiesReliabilityLayer() {
  ...
  assert.equal(health.version, "v4");
  ...
  assert.equal(timerResult.version, "v3", "timer cleanup results are not mislabeled as health v4");
