...
test("faceRecognition v108 validates and atomically creates recharge gifts", () => {
  const cloud = read("cloudfunctions/faceRecognition/index.js");
  assert.match(cloud, /PHOTO_ONLY_FUNCTION \? "v9" : "v108"/);
...
test("staff detail readers expose the immutable gift lines", () => {
  ...
  assert.match(cloud, /const FUNCTION_VERSION = "v80"/);
