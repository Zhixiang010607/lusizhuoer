...
for (const page of pages) {
  ...
  includes(html, "styles.css?v=0.15.35", `${page} responsive store selector styles`);
...
for (const file of fs.readdirSync(root).filter((file) => file.endsWith(".html") && read(file).includes("auth-ui.js?v="))) {
  includes(read(file), "auth-ui.js?v=0.18.4", `${file} auth cache key`);
}
...
(async () => {
  ...
  includes(cloud, 'const FUNCTION_VERSION = PHOTO_ONLY_FUNCTION ? "v3" : "v57"', "deployable service versions");
