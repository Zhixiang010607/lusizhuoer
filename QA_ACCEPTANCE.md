...
| `pages/inactive-customers/index` | 总部／门店手填未核销天数；正常核销 → 体验核销 → 客户建档固定优先级；结果按“全部项目余次为 0”和“任意项目余次非 0”两类客户分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 服务端客户级分类、上海自然日、封存客户排除、角色范围、稳定游标、双分类首屏与完整导出单语句快照、手机／iPad 紧凑表格和两种格式 1000 条上限 `PASS-AUTO`；内置中文字体后的官方预览分包为 773.1 KB，低于 2 MB `PASS-BUILD`；仓库目标为待部署 `faceRecognition v113`，线上仍为已验证的 v112，小程序仍为已上传开发版 `0.2.60` |
| `pages/low-balance-customers/index` | 总部／门店选择单个或全部项目并手填余次阈值；结果按“客户＋项目余次为 0”和“非 0 且低于阈值”分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 客户＋项目分类、严格小于、从未开卡不误报、封存客户排除、活跃项目、角色范围、稳定游标、双分类首屏与完整导出单语句快照、手机／iPad 紧凑表格和两种格式 1000 条上限 `PASS-AUTO`；内置中文字体后的官方预览分包为 774.9 KB，低于 2 MB `PASS-BUILD`；仓库目标为待部署 `faceRecognition v113`，线上仍为已验证的 v112，小程序仍为已上传开发版 `0.2.60` |
| `pages/rating-analysis/index` | 总部／门店按门店、项目、老师、上海业务时间和 0—5 分任意多选查询；0 为未评价、其余按三项最低分归档；仅已评价的 1—5 分分布与独立评价覆盖率双饼图、20 条分页、工单／客户详情，以及当前全部结果按门店分组导出 PDF／Excel | 汇总、图表、列表和完整导出单语句快照、无 `conic-gradient` 的手机／iPad 圆环、放大自适应三列表格、两种格式 1000 条上限及嵌入字体 PDF `PASS-AUTO`／`PASS-VISUAL`；官方预览分包为 779.1 KB，低于 2 MB `PASS-BUILD`；仓库目标为待部署 `customerRating v7`，小程序仍为已上传开发版 `0.2.60` |
