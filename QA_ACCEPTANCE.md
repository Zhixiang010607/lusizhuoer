...
| `pages/inactive-customers/index` | 总部／门店手填未核销天数；正常核销 → 体验核销 → 客户建档固定优先级；结果按“全部项目余次为 0”和“任意项目余次非 0”两类客户分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 服务端客户级分类、上海自然日、封存客户排除、角色范围、稳定游标、双分类请求隔离、手机／iPad 紧凑表格和两种格式 10000 条上限 `PASS-AUTO`；当前目标为 `faceRecognition v112`／小程序 `0.2.60`，尚未部署，真实数据库结果仍待专用角色复验 |
| `pages/low-balance-customers/index` | 总部／门店选择单个或全部项目并手填余次阈值；结果按“客户＋项目余次为 0”和“非 0 且低于阈值”分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 客户＋项目分类、严格小于、从未开卡不误报、封存客户排除、活跃项目、角色范围、稳定游标、双分类请求隔离、手机／iPad 紧凑表格和两种格式 10000 条上限 `PASS-AUTO`；当前目标为 `faceRecognition v112`／小程序 `0.2.60`，尚未部署，真实数据库结果仍待专用角色复验 |
| `pages/rating-analysis/index` | 总部／门店按门店、项目、老师、上海业务时间和 0—5 分任意多选查询；0 为未评价、其余按三项最低分归档；仅已评价的 1—5 分分布与独立评价覆盖率双饼图、20 条分页、工单／客户详情，以及当前全部结果按门店分组导出 PDF／Excel | 既有评分统计、图表和手机／iPad 布局继续 `PASS-AUTO`／`PASS-VISUAL`；新增完整分页拉取、快照总数复核和两种格式 10000 条上限 `PASS-AUTO`；当前小程序目标为 `0.2.60`，尚未上传，真实角色数据仍待复验 |
...
14. 2026-09-05 仓库目标 `faceRecognition v112`／小程序 `0.2.60` 将活跃预警按客户的全部项目余次是否全为 0 分成两类，将余次预警按客户＋项目余次为 0 或非 0 且低于阈值分成两类；四个分类各自拥有 20 条分页和直接跳页。运营核心指标查询中的活跃预警、余次预警和评价分析均可把当前筛选的完整结果按门店分组导出 PDF 或 Excel，单种格式上限 10000 条。微信开发者工具真实 AppID 编译成功，10000 条 PDF 本机生成验证为 501 页、约 6.8 MB；本轮不改数据库结构、没有新 SQL，云函数、小程序均尚未上传或发布。
