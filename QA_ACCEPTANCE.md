...
| `pages/rating-analysis/index` | 总部／门店按门店、项目、老师、上海业务时间和 0—5 分任意多选查询；0 为未评价、其余按三项最低分归档；仅已评价的 1—5 分分布与独立评价覆盖率双饼图、20 条分页和工单／客户详情 | 最低分 SQL、无老师两项取低、0 分未评价、门店锁定、分数 OR 筛选、1—5 分以已评价总数为分母、图表分母不受分数勾选影响、页面内圆环随布局显示、三列表格全宽内容自适应、手机按钮两行三列／iPad 一行六列无重合、手机单列／iPad 双列图表及横向表格 `PASS-AUTO`／`PASS-VISUAL`；`customerRating v6` 已通过线上健康检查，页面已进入 `0.2.59` 开发版，真实角色数据仍待复验 |
