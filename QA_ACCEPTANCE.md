...
| `pages/inactive-customers/index` | 总部／门店手填未核销天数；正常核销 → 体验核销 → 客户建档固定优先级；结果按“全部项目余次为 0”和“任意项目余次非 0”两类客户分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 服务端客户级分类、上海自然日、封存客户排除、角色范围、稳定游标、双分类请求隔离、手机／iPad 紧凑表格和两种格式 10000 条上限 `PASS-AUTO`；`faceRecognition v112` 已部署并通过健康检查，`0.2.60` 已上传开发版；总部生产空结果查询的路由、完成态和无错误状态 `PASS-LIVE`，有数据分类仍待真实业务记录复验 |
| `pages/low-balance-customers/index` | 总部／门店选择单个或全部项目并手填余次阈值；结果按“客户＋项目余次为 0”和“非 0 且低于阈值”分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 客户＋项目分类、严格小于、从未开卡不误报、封存客户排除、活跃项目、角色范围、稳定游标、双分类请求隔离、手机／iPad 紧凑表格和两种格式 10000 条上限 `PASS-AUTO`；`faceRecognition v112` 已部署并通过健康检查，`0.2.60` 已上传开发版；总部生产空结果查询的路由、完成态和无错误状态 `PASS-LIVE`，有数据分类仍待真实业务记录复验 |
| `pages/rating-analysis/index` | 总部／门店按门店、项目、老师、上海业务时间和 0—5 分任意多选查询；0 为未评价、其余按三项最低分归档；仅已评价的 1—5 分分布与独立评价覆盖率双饼图、20 条分页、工单／客户详情，以及当前全部结果按门店分组导出 PDF／Excel | 既有评分统计、图表和手机／iPad 布局继续 `PASS-AUTO`／`PASS-VISUAL`；新增完整分页拉取、快照总数复核和两种格式 10000 条上限 `PASS-AUTO`；`0.2.60` 已上传开发版，真实有数据导出仍待业务记录复验 |
