...
| `pages/order-detail/index` | 四类工单精确回读、完整单号单行、客户入口、五照片位、重试／补图、原图相册、PDF／图片导出，以及核销工单底部客户评价 | 原有权限、照片和单据契约继续 `PASS-AUTO`；评价暂无／星级／留言、总部与所属门店二维码及老师无二维码导出 `PASS-AUTO`；当前代码已进入 `0.2.55` 开发版本，真实照片、评价、相册、补图和三角色导出仍需真机／专用账号复验 |
