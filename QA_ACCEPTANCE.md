...
4. 2026-08-25 已重新扫码登录微信开发者工具并完成 `build-npm`；新增客户／充值／核销查询路由均在模拟器编译、渲染且没有页面异常。登录页的 Logo 视觉衍生亮色背景、弧形居中框、框内标题／字段／按钮已经完成新一轮开发者工具截图复验。真实总部业务会话在本轮重编译后尚未重新输入，因此新增筛选与工单详情仍需完成真实数据库只读回归；不得把仅渲染成功写成 `PASS-LIVE`。
