...
| `pages/inactive-customers/index` | 总部／门店手填未核销天数；正常核销 → 体验核销 → 客户建档固定优先级；结果按“全部项目余次为 0”和“任意项目余次非 0”两类客户分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 服务端客户级分类、上海自然日、封存客户排除、角色范围、稳定游标、双分类首屏与完整导出单语句快照、手机／iPad 紧凑表格和两种格式 1000 条上限 `PASS-AUTO`；官方上传分包为 773.1 KB，低于 2 MB `PASS-BUILD`；`faceRecognition v113` 已上传且状态 `Active`，线上回下载文件与仓库一致，运行时 `health` 因控制台编辑器加载失败仍待补；小程序开发版已上传为 `0.2.61` |
| `pages/low-balance-customers/index` | 总部／门店选择单个或全部项目并手填余次阈值；结果按“客户＋项目余次为 0”和“非 0 且低于阈值”分别显示、独立 20 条分页与直接跳页；当前全部结果按门店分组导出 PDF／Excel | 客户＋项目分类、严格小于、从未开卡不误报、封存客户排除、活跃项目、角色范围、稳定游标、双分类首屏与完整导出单语句快照、手机／iPad 紧凑表格和两种格式 1000 条上限 `PASS-AUTO`；官方上传分包为 774.9 KB，低于 2 MB `PASS-BUILD`；`faceRecognition v113` 已上传且状态 `Active`，线上回下载文件与仓库一致，运行时 `health` 因控制台编辑器加载失败仍待补；小程序开发版已上传为 `0.2.61` |
| `pages/rating-analysis/index` | 总部／门店按门店、项目、老师、上海业务时间和 0—5 分任意多选查询；0 为未评价、其余按三项最低分归档；仅已评价的 1—5 分分布与独立评价覆盖率双饼图、20 条分页、工单／客户详情，以及当前全部结果按门店分组导出 PDF／Excel | 汇总、图表、列表和完整导出单语句快照、无 `conic-gradient` 的手机／iPad 圆环、放大自适应三列表格、两种格式 1000 条上限及嵌入字体 PDF `PASS-AUTO`／`PASS-VISUAL`；官方上传分包为 779.1 KB，低于 2 MB `PASS-BUILD`；`customerRating v7` 已上传且状态 `Active`，线上回下载文件与仓库一致，运行时 `health` 因控制台编辑器加载失败仍待补；小程序开发版已上传为 `0.2.61` |
...
12. 2026-09-03 已将小程序 `0.2.58` 上传为开发版本，主包 `1,594,721` 字节、总包 `2,033,118` 字节。评价分析继续允许 0—5 分查询，但评分分布仅显示已评价的 1—5 分并以已评价总数为分母；0 分未评价只进入覆盖率图。饼图、中心数字、图例、比例和说明文字均已放大。iPhone 12/13 Pro 实测评分按钮两行三列，iPad Pro 10.5-inch 实测一行六列，两端按钮、饼图和图例均无重合或越界；279 项自动化通过。本轮不改云函数、数据库或网页，没有新 SQL；`0.2.58` 尚未设为体验版、提交审核或正式发布。
13. 2026-09-03 已将小程序 `0.2.59` 上传为开发版本，主包 `1,594,721` 字节、总包 `2,033,657` 字节。评价分析圆环改为随页面布局的普通视图绘制，消除原生 Canvas 重排后的空白和渲染层漂移；评分／状态、数量、占比三列复用其他查询结果的全宽内容自适应规则，数量与百分比分列并继续放大字体。iPhone 15 Pro Max 实测手机上下单列、按钮两行三列；768×1024 iPad 实测左右双列、按钮一行六列；两端圆环均显示，三列表格按一位评分／六位数量／百分比自动分配列宽且不越出卡片。279 项自动化通过。本轮不改云函数、数据库或网页，没有新 SQL；`0.2.59` 尚未设为体验版、提交审核或正式发布。
14. 2026-09-05 `faceRecognition v112` 已通过官方 CloudBase CLI 上传，回执 3 个文件／84.0 KB、`success=true`，函数状态为 `Active`；小程序真实调用 `health` 返回 `ok=true`、`version=v112`，私有照片存储检查正常。小程序 `0.2.60` 已通过官方 CLI 上传开发版本，主包 `1,603,632` 字节、总包 `2,064,820` 字节。活跃预警按客户的全部项目余次是否全为 0 分成两类，余次预警按客户＋项目余次为 0 或非 0 且低于阈值分成两类；四个分类各自拥有 20 条分页和直接跳页。运营核心指标查询中的活跃预警、余次预警和评价分析均可把当前筛选的完整结果按门店分组导出 PDF 或 Excel，单种格式上限 10000 条；10000 条 PDF 本机生成为 501 页、约 6.8 MB。已登录总部会话对两个生产查询完成空结果只读复验，不建立测试客户。本轮不改数据库结构、没有新 SQL；公众平台禁止当前自动化会话进入，`0.2.60` 尚待管理员手动设为体验版，未提交审核或正式发布。
15. 2026-09-05 `faceRecognition v113` 与 `customerRating v7` 已通过微信开发者工具官方 CloudBase CLI 上传，平台分别回执 3 个文件／85.2 KB 和 3 个文件／10.9 KB、`success=true`，只读函数信息均为 `Active`；线上回下载的 `index.js`、`README.md`、`package.json` 与仓库 SHA-256 全部一致。CloudBase 控制台的测试编辑器本次持续“加载失败”，因此不把状态和文件一致性冒充为运行时 `health`。小程序 `0.2.61` 第二次官方 CLI 上传成功，主包 `1,607,496` 字节、总包 `4,360,674` 字节，活跃预警、余次预警和评价分析分包分别为 `791,616`、`793,491`、`797,773` 字节，均低于 2 MB；首次上传只发生微信接口超时，没有代码校验错误。该版本把三页查询与导出切换到数据库快照，运营 PDF／Excel 上限固定为 1000 条，并内置压缩中文字体、兼容旧 iPad 图表和前后台屏幕常亮。本轮不改数据库、没有新 SQL、不发布静态网页；公众平台禁止自动化访问，`0.2.61` 尚待管理员在版本管理中手动设为体验版，未提交审核或正式发布。
