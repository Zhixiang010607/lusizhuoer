...
| 核销查询与详情 | `verification-query.html` 条件、类型、客户查询、重置、分页；`verification-detail.html` 5 个照片位、原图、重读、PDF、图片、单张下载，以及工单底部客户评价 | 原有详情 `PASS-LIVE`；评价的暂无／星级／留言、三角色只读边界和门店导出二维码 `PASS-AUTO`，待 068、云函数和静态网页部署后真实角色复验 |
| 客户扫码评价 | `rating.html` 匿名会话、门店环境／老师服务／整体体验 1—5 星、500 字留言和完成态 | 手机／iPad 视觉及公开令牌、单次提交、无老师时省略老师评分 `PASS-AUTO`；匿名登录、网关权限和真实二维码扫码 `NEEDS-CONSOLE-SETUP` |
...
5. 客户评价迁移 068 尚未在 CloudBase 执行，`customerRating v1` 尚未上传，匿名登录／网关权限尚未配置，静态 `rating.html` 和小程序 `0.2.49` 也尚未发布；当前只计本地自动化与视觉验收，不能宣称线上客户可扫码评价。
