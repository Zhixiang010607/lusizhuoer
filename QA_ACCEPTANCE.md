...
| 核销查询与详情 | `verification-query.html` 条件、类型、客户查询、重置、分页；`verification-detail.html` 5 个照片位、原图、重读、PDF、图片、单张下载，以及工单底部客户评价 | 原有详情 `PASS-LIVE`；评价的暂无／星级／留言、三角色只读边界、总部与所属门店导出二维码及老师无二维码 `PASS-AUTO`；068、`customerRating v2` 和静态网页已部署，仍待总部／门店／老师真实角色复验 |
...
| `pages/order-detail/index` | 四类工单精确回读、完整单号单行、客户入口、五照片位、重试／补图、原图相册、PDF／图片导出，以及核销工单底部客户评价 | 原有权限、照片和单据契约继续 `PASS-AUTO`；评价暂无／星级／留言、总部与所属门店二维码及老师无二维码导出 `PASS-AUTO`；`0.2.50` 已上传为开发版本，真实照片、评价、相册、补图和三角色导出仍需真机／专用账号复验 |
