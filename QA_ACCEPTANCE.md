...
| 核销查询与详情 | `verification-query.html` 条件、类型、客户查询、重置、分页；`verification-detail.html` 5 个照片位、原图、重读、PDF、图片、单张下载，以及工单底部客户评价 | 原有详情 `PASS-LIVE`；评价的暂无／星级／留言、三角色只读边界、总部与所属门店导出二维码及老师无二维码 `PASS-AUTO`；068、`customerRating v3` 和静态网页已部署，首次签发写后读误报已修复，仍待总部／门店／老师真实角色复验 |
