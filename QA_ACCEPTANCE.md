...
| `pages/inactive-customers/index` | 总部／门店手填未核销天数；正常核销 → 体验核销 → 客户建档固定优先级；客户／门店／间隔时间／计算起点／上次核销五列、最久未核销优先、分页、直接跳页和客户详情 | 服务端固定阶梯、上海自然日、封存客户排除、角色范围、稳定游标、请求隔离及手机／iPad 紧凑表格 `PASS-AUTO`；`faceRecognition v110` 已上传，真实数据库结果仍待专用角色复验 |
