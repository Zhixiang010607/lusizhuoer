...
| 核销查询与详情 | `verification-query.html` 条件、类型、客户查询、重置、分页；`verification-detail.html` 5 个照片位、原图、重读、PDF、图片、单张下载，以及工单底部客户评价 | 原有详情 `PASS-LIVE`；评价的暂无／星级／留言、三角色只读边界、总部与所属门店导出二维码及老师无二维码 `PASS-AUTO`；068 已执行，当前服务目标为 `customerRating v4`，兼容写入成功但 `RETURNING` 为空的首次签发与令牌更新，仍待总部／门店／老师真实角色复验 |
| 客户扫码评价 | `rating.html` 匿名会话、门店环境／老师服务／整体体验 1—5 星、500 字留言和完成态 | 手机／iPad 视觉及公开令牌、单次提交、无老师时省略老师评分 `PASS-AUTO`；匿名登录、OPA 网关、函数安全规则和静态页面已部署，匿名请求进入评价函数、伪造令牌受控拒绝且同会话调用员工函数被网关拒绝 `PASS-LIVE`；待真实工单二维码提交复验 |
...
| `pages/inactive-customers/index` | 总部／门店手填未核销天数；正常核销 → 体验核销 → 客户建档固定优先级；全部／指定／本店范围、汇总、分页、直接跳页和客户详情 | 服务端固定阶梯、上海自然日、封存客户排除、角色范围、稳定游标、请求隔离及手机／iPad 紧凑表格 `PASS-AUTO`；真实数据库结果待 `faceRecognition v110` 部署后复验 |
| `pages/low-balance-customers/index` | 总部／门店选择单个或全部项目并手填余次阈值；只查已开卡权威余额、0 次命中、从未开卡不误报、分页和客户详情 | 服务端严格小于、封存客户排除、活跃项目、角色范围、稳定游标、请求隔离及手机／iPad 紧凑表格 `PASS-AUTO`；真实数据库结果待 `faceRecognition v110` 部署后复验 |
...
| `pages/order-detail/index` | 四类工单精确回读、完整单号单行、客户入口、五照片位、重试／补图、原图相册、PDF／图片导出，以及核销工单底部客户评价 | 原有权限、照片和单据契约继续 `PASS-AUTO`；评价暂无／星级／留言、总部与所属门店二维码及老师无二维码导出 `PASS-AUTO`；`0.2.51` 已上传为开发版本，真实照片、评价、相册、补图和三角色导出仍需真机／专用账号复验 |
...
8. 2026-09-02 当前待部署目标为 `faceRecognition v110` 与小程序 `0.2.54`：增加仅面向总部／门店、永久排除封存客户的“未核销客户”和“低余次客户”查询。专项自动化已覆盖固定核销优先级、权威已开卡余额、单个／全部活跃项目、门店权限、稳定分页和手机／iPad 布局；本轮不需要新 SQL。云函数上传、`health`、小程序开发版上传和真实角色数据库结果须按最终交付状态单独记录，未完成前不得标为 `PASS-LIVE`。
