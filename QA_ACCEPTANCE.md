...
| 核销查询与详情 | `verification-query.html` 条件、类型、客户查询、重置、分页；`verification-detail.html` 5 个照片位、原图、重读、PDF、图片、单张下载，以及工单底部客户评价 | 原有详情 `PASS-LIVE`；评价的暂无／星级／留言、三角色只读边界和门店导出二维码 `PASS-AUTO`；068、云函数和静态网页已部署，仍待门店／老师专用账号真实角色复验 |
...
| `pages/order-detail/index` | 四类工单精确回读、完整单号单行、客户入口、五照片位、重试／补图、原图相册、PDF／图片导出，以及核销工单底部客户评价 | 原有权限、照片和单据契约继续 `PASS-AUTO`；评价暂无／星级／留言、门店二维码与老师／总部无二维码导出 `PASS-AUTO`；真实照片、评价、相册、补图和导出需真机／专用账号复验 |
...
5. 2026-09-01 已执行客户评价迁移 068 并取得 9 行 `READY`，部署并通过 `customerRating v1` 健康检查，开启匿名登录，合并仅放行评价路由的 OPA 策略和 `customerRating` 的 `"auth != null"` 函数安全规则，发布含 `rating.html` 的 12 个静态文件，并把小程序 `0.2.49` 上传为开发版本。线上匿名边界已经回归：评价请求到达函数、伪造令牌显示“评价链接无效”，同一匿名会话调用 `staffAccount` 仍被拒绝。仍须用真实工单二维码完成一次生产提交，并用门店／老师专用账号复验查看与导出；开发版本尚未设为体验版、提交审核或正式发布。
