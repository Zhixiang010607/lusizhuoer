...
3. 小程序 `0.2.10` 已于 2026-08-25 上传为体验版，结果为 `UPLOADED-EXPERIENCE`；总包 `1,722,970` 字节、主包 `1,486,513` 字节。总部老师主页已移除重复的独立档案卡，老师编号和联系电话并入顶部档案头；每张已配置产品卡只保留“每月基础／本月已体验”两项摘要。保存新临时密码、封存或激活老师、配置产品、单独充值体验次数和额度流水仍完整保留。尚未提交微信审核或正式发布。本轮没有上传云函数、运行 SQL 或发布网站，代码推送与这些状态仍分别确认。
