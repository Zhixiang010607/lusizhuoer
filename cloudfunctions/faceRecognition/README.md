...
- `getTeacherWorkspace`：仅允许活跃老师读取本人基础资料；本人业务列表仍只查询 `teacher_id` 等于本人的充值／核销工单，按服务端页码读取且每页最多 100 条。读取精确详情时，服务端先确认该工单客户属于当前老师的有效客户关系，再允许查看其他老师为同一客户提交的工单，并返回工单实际业务老师，绝不使用当前登录老师覆盖显示。
