...
`v114` 接入魔法柔肤厂家确认的设备身份格式：项目固定映射为 `device_type=LASER-BLE`，二维码和实时设备信息接受 `device_id=LA` 加 12 位大写十六进制，BLE 广播名固定为 `LA-` 加设备编号末 6 位。服务端签名使用项目映射后的规范类型；旧 `NCM` 编号和其他项目原有拼音类型继续兼容。生产数据库必须先执行迁移 069。
...
主档，不接收图片或建立老师人脸。`faceRecognition v114` 不接受老师创建、照片检测、
...
- `faceRecognition v114` 暂时保留旧客户端的 `DIRECT`／精确 `FUNCTION` 回退实现；当前生产前端不调用这组兼容照片动作。两个函数均不会接受客户端指定的桶或对象路径，独立服务签名失败时会取消精确任务而不转入函数字节中转。
...
上传包文件名必须为 `faceRecognition-v114.zip`。把本目录中的 `index.js`、`package.json` 和 `README.md` 放在 ZIP 根目录；不要再套一层目录。函数入口为 `index.main`，部署时安装 `package.json` 依赖。交付前必须回读 ZIP 根目录的 README 与运行时代码，确认两者均为 `v114`。
...
4. 执行迁移 050 的 7 段控制台 SQL并确认只读验收全部 `READY`，再按 053 指引退役旧老师人脸 Saga。`faceRecognition v114` 不依赖迁移 051／052，也不需要老师人脸操作恢复 Timer。
5. 完整执行迁移 054（CloudBase SQL 编辑器使用 `054-01-teacher-only-customer-face-experience.sql`），把新体验核销切换为老师账号赠送、客户人脸凭证。
6. 将 `faceRecognition` 执行超时设为 **90 秒**；准备 `faceRecognition v114`、`verificationPhoto v11`、`staffAccount v81` 和 `teacherCreate v6`，但等 060—069 验收完成后再统一部署。`teacherCreate` 可设为 **60 秒、至少 256 MB**，且不再需要任何 FACE 或照片桶变量。
...
13. 执行并验收 068，配置至少 32 字节的 `CUSTOMER_RATING_SIGNING_KEY` 及实际 `rating.html` 地址 `CUSTOMER_RATING_BASE_URL`，准备 `customerRating-v7.zip`。如果旧版 066 曾建立第三张设备注册表，先执行 `066-02-retire-legacy-device-registry.sql`；随后执行并验收 069，再部署 `faceRecognition v114` 及同轮配套函数，分别调用 `health` 核对实际版本，最后发布包含新 BLE 识别规则的小程序。

当前切换顺序为“确认 048—050 与 053 已完成 → 依次执行并验收 054—065 → 执行 066 并配置 `BLE_AUTH_SIGNING_KEY` → 低峰期执行并验收 067 → 部署
`faceRecognition v114`、同轮配套的 `verificationPhoto v11`、`staffAccount v81`、`teacherCreate v6`、`customerRating v7` → 分别 health → 运行 065—069 只读验收 →
...
- `queryInactiveVerificationCustomers`：总部与门店共用的“活跃预警”查询，只读取 `customer_status='ACTIVE'` 的正常在档客户。总部可查全部或一个真实门店，门店始终锁定本店，老师无权调用。`minimumDays` 必须是 `1—3650` 的整数上海自然日；每位客户只要存在任意已完成正常核销就取最新 `NORMAL`，完全没有正常核销时才取最新 `EXPERIENCE`，两类都没有时取建档时间。充值、退费和产品购买不参与起点。`balanceCategory` 可为 `ZERO`、`NONZERO` 或兼容旧客户端的 `ALL`：只要权威余额表存在任意 `remaining_count <> 0` 的项目，该客户属于 `NONZERO`，否则属于 `ZERO`；没有余额行也属于 `ZERO`。分类各自按起点最早优先，以 `(baseline_at,customer_id)` 稳定分页，汇总同时返回两类客户数。
- `queryLowBalanceCustomers`：总部与门店共用的“余次预警”查询，只读取正常在档客户与当前活跃项目。`remainingBelow` 必须是 `1—999999` 的整数，可提交一个真实项目 ID 或留空表示全部项目。接口直接读取 `customer_product_balances`，要求 `total_recharge_count > 0` 且 `remaining_count < remainingBelow`；余额为 0 的已开卡项目会命中，从未开卡的项目不会被合成 0 次。结果对象固定为客户×项目卡项；`balanceCategory=ZERO` 只返回余次等于 0 的卡项，`NONZERO` 只返回余次不等于 0 且低于阈值的卡项，`ALL` 保持旧客户端兼容。两类各自按 `(remaining_count,customer_id,product_id)` 稳定分页，汇总同时返回两类卡项数。
