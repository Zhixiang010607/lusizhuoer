...
- 原图和缩略图设置私有长期缓存；数据库从不保存签名 URL。详情首屏只签名并懒加载最多 4 张缩略图，点击后才为单张原图生成 5 分钟临时地址。
...
- `createVerificationApplication`：门店与老师均支持正常及补录核销；老师必须先选择本次办理门店，服务端强制 `teacher_id` 为当前登录老师。两种身份均必须先确认目标门店活跃客户、有效余额并完成该客户的 1:1 人脸验证；服务端同时校验未过期、未消费、属于同一提交账号／门店／客户／人脸请求的照片令牌，并通过迁移 037 的数据库函数原子创建工单和绑定人脸照片。补录核销必须填写原因、写入 `PENDING` 并等待总部或运营审核，不发送设备开启信号。
...
- `getVerificationPhotos`：总部可查看全部核销照片，门店只能查看本店，老师只能查看本人绑定工单，运营只能查看审核范围内的补录核销。一次返回最多 4 张短时缩略图和服务端计算的上传权限；不预签原图。
- `getVerificationPhotoOriginalUrl`：再次校验相同工单权限后，只为用户点击的一个照片位签发原图短时地址。
- `uploadVerificationExtraPhoto`：仅允许核销单 `submitted_by_account_id` 对应的真实登录账号，在 `submitted_at + 24 hours` 之前上传或替换照片位 1--3；总部、运营、其他门店或其他老师均不可写。数据库触发器和云函数双重校验，照片位 0 的人脸凭证永久不可替换。
