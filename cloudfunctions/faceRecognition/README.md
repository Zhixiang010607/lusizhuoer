...
13. 执行并验收 068，配置至少 32 字节的 `CUSTOMER_RATING_SIGNING_KEY` 及实际 `rating.html` 地址 `CUSTOMER_RATING_BASE_URL`，准备 `customerRating-v4.zip`。如果旧版 066 曾建立第三张设备注册表，先执行 `066-02-retire-legacy-device-registry.sql`；部署 `faceRecognition v110` 及同轮配套函数，分别调用 `health` 核对实际版本，再发布包含 `rating.html` 的网页和小程序并强制刷新。

当前切换顺序为“确认 048—050 与 053 已完成 → 依次执行并验收 054—065 → 执行 066 并配置 `BLE_AUTH_SIGNING_KEY` → 低峰期执行并验收 067 → 部署
`faceRecognition v110`、同轮配套的 `verificationPhoto v10`、`staffAccount v81`、`teacherCreate v6`、`customerRating v4` → 分别 health → 运行 065—068 只读验收 →
...
- `queryInactiveVerificationCustomers`：总部与门店共用的“活跃预警”查询，只读取 `customer_status='ACTIVE'` 的正常在档客户。总部可查全部或一个真实门店，门店始终锁定本店，老师无权调用。`minimumDays` 必须是 `1—3650` 的整数上海自然日；每位客户只要存在任意已完成正常核销就取最新 `NORMAL`，完全没有正常核销时才取最新 `EXPERIENCE`，两类都没有时取建档时间。充值、退费和产品购买不参与起点；结果按起点最早优先，以 `(baseline_at,customer_id)` 稳定分页。
- `queryLowBalanceCustomers`：总部与门店共用的“余次预警”查询，只读取正常在档客户与当前活跃项目。`remainingBelow` 必须是 `1—999999` 的整数，可提交一个真实项目 ID 或留空表示全部项目。接口直接读取 `customer_product_balances`，要求 `total_recharge_count > 0` 且 `remaining_count < remainingBelow`；余额为 0 的已开卡项目会命中，从未开卡的项目不会被合成 0 次。结果按 `(remaining_count,customer_id,product_id)` 稳定分页，并分别返回匹配卡项、客户、项目和 0 次余额总数。
