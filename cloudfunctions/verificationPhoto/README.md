...
4. 依次执行 `049-01` 至 `049-13` 和只读的 `049-readonly-verify.sql`，再执行 `050-01` 至 `050-07`、`051-01` 至 `051-10`、`052-01-auth-create-receipt.sql` 及各自只读验收；将 `faceRecognition` 执行超时固定为 90 秒（不得更高）、`staffAccount` 固定为 600 秒，最后部署 `faceRecognition v75` 与 `staffAccount v64`，并创建 `reconcile-teacher-face-operations` 的每 1 分钟 Timer。
