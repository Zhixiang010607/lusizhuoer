...
2. 部署当前 `faceRecognition-v101.zip` 与 `staffAccount-v79.zip`，分别调用 `health` 确认 `v101`、`v79`。在仅限总部使用、已加载当前 `cloudbase-phone-auth.js` 的临时维护页面中，以已登录总部身份执行 `await CloudBasePhoneAuth.retireOperationAccounts()`；必须等待成功封锁旧运营账号的 CloudBase 凭据。该维护页不是最终静态发布。
...
4. 依次执行并验收 049、050；部署不再读写旧 Saga 的 `faceRecognition v101`、`staffAccount v79` 和 `teacherCreate v6` 后执行 053 并确认 7 行全部 `RETIRED`。继续依次执行并验收 054—064；删除旧 `reconcile-teacher-face-operations` Timer，保留老师额度月初 Timer。
