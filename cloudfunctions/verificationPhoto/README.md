...
7. 评价同轮还必须执行并验收 068、配置至少 32 字节的 `CUSTOMER_RATING_SIGNING_KEY` 及实际 `rating.html` 地址 `CUSTOMER_RATING_BASE_URL`、上传 `customerRating-v3.zip` 并确认 `health` 为 `v3` 且 `configured=true`。`verificationPhoto v10` 不读取或写入评价。
