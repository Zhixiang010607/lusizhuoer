...
3. 客户评价作为独立同轮服务部署：执行并验收 068，配置至少 32 字节的 `CUSTOMER_RATING_SIGNING_KEY` 及实际 `rating.html` 地址 `CUSTOMER_RATING_BASE_URL`，上传 `customerRating-v3.zip` 并确认其 `health`。`staffAccount v81` 不签发或写入评价。
