(() => {
  ...
  function previewCustomer(customer) { candidateCustomer = customer; $("teacherCustomerPreview").innerHTML = `<div class="customer-core-card"><div class="customer-core-heading"><span>客户身份确认</span><strong>${escapeHtml(customer.name)}</strong></div><div class="customer-core-facts"><div><span>姓名</span><strong>${escapeHtml(customer.name)}</strong></div><div><span>生日</span><strong>${escapeHtml(formatBirthday(customer.birthday))}</strong></div><div><span>客户编号</span><strong>${escapeHtml(customer.id)}</strong></div></div></div>`; $("teacherConfirmCustomer").disabled = false; }
  function findSelectedCustomer() { const id = $("teacherCustomerSelect").value, customer = storeCustomers().find((item) => item.id === id); if (!customer?.birthday) { resetCustomer(); $("teacherCustomerPreview").innerHTML = `<div class="lookup-placeholder error"><strong>未能确认客户</strong><span>请选择具有完整生日资料的客户。</span></div>`; return; } previewCustomer(customer); }
  ...
  $("teacherSelectBirthday").type = "text";
  ...
  $("teacherCustomerSelect").addEventListener("change", () => { const customer = storeCustomers().find((item) => item.id === $("teacherCustomerSelect").value); $("teacherSelectBirthday").value = formatBirthday(customer?.birthday, ""); resetCustomer(); }); $("teacherSelectCustomer").addEventListener("click", findSelectedCustomer); $("teacherManualCustomer").addEventListener("click", findManualCustomer); $("teacherConfirmCustomer").addEventListener("click", confirmCustomer);
