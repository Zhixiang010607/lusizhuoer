...
- 2026-08-25 最终上传的体验版 `0.2.4` 总包为 `1,722,399` 字节，主包为 `1,486,251` 字节，均低于微信限制。启动页和共用 WXSS 页面留在主包，其余 12 个业务页使用原目录普通分包；分包不得改成 `independent`，否则会失去主包的会话、服务、组件和全局样式。上传配置会过滤不可达 npm 产物、sourcemap、类型声明和本地依赖元数据，测试会保留至少 200KB 的主包预算余量。
