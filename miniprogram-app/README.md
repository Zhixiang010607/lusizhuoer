...
- 小程序登录页不使用人物照、图形头像、装饰线或宣传文案，只显示“露思卓儿”四字品牌；视觉为官网式暖白、深灰蓝、冷灰平面体系，不使用卡片、投影、渐变、金色或绿色强调，微信手机号登录采用同色系描边次按钮。总部、门店、老师首页在相同手机内容视口下逐项复现网页版的模块顺序、字号、间距、颜色、圆角、按钮网格、表格列宽和滚动方式；微信状态栏、右上角胶囊、原生导航栏与浏览器地址栏属于平台差异。WXML/WXSS、小程序会话和微信能力仍与网页 HTML/CSS、浏览器会话物理隔离，禁止跨目录导入客户端源码。
