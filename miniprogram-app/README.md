...
8. 总部产品、门店、老师管理：名称／编号／手机号查询、活跃与封存分区、新增资料和登录账号、详情、老师体验额度读取，以及由数据库回读确认的封存／激活。
...
- 产品、门店、老师管理分别检查名称／编号／手机号查询、活跃／封存分区、新增表单和详情；任何创建或状态切换只在专用测试数据上提交，并在提交后重新查询数据库确认。
