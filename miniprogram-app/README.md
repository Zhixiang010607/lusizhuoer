...
3. 总部／门店／老师三种首页：按网页版同一手机视口复现移动导航、标题栏、信息卡、时间筛选、四类业务汇总、横向明细表、客户表和分页；老师主页的三项基础信息同一横排，每个体验项目也以“项目／可用／每月基础／本月已用”单行展示。老师主页不再提前选择门店；点击建档、充值／退费、正常核销或体验核销进入具体业务页后，第一步重新读取并选择门店，再在该门店全部活跃客户的纵向滚动列表中分批加载、连续选择。姓名＋生日继续作为并行精确查询入口，客户主页继续使用小程序原生路由。门店账号不显示门店选择器，继续使用同一组件读取其 UID 绑定门店的全部活跃客户。
...
- 2026-08-25 最终上传的体验版 `0.2.5` 总包为 `1,722,708` 字节，主包为 `1,486,251` 字节，均低于微信限制。该版修正客户备注右侧溢出并统一客户留言、留言输入区的暖金配色。启动页和共用 WXSS 页面留在主包，其余 12 个业务页使用原目录普通分包；分包不得改成 `independent`，否则会失去主包的会话、服务、组件和全局样式。上传配置会过滤不可达 npm 产物、sourcemap、类型声明和本地依赖元数据，测试会保留至少 200KB 的主包预算余量。
