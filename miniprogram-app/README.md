...
8. 总部管理：门店按名称／联系人电话、老师按姓名／联系电话查询，查询结果与活跃／封存分区均使用网页版手机端同款横向表格；新增进入独立页面。门店主页包含基础资料、时间范围、项目汇总、四类有效业务明细、精确工单／客户跳转、分页、活跃／封存客户和数据库回读确认的状态操作；老师主页包含档案、临时密码重置、账号状态、体验总览、项目累计、配置／充值／删除额度和完整历史。产品管理使用独立“全部产品 → 产品创建 → 产品单据模板”流程，模板支持共用原始 LOGO、两组共用说明、保存后数据库复核、产品封存／激活和核销／充值 PDF 与图片四种样例预览及导出；LOGO 区不显示文件名、编号、大小、像素或压缩说明。
