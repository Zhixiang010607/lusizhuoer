...
4. 总部／门店三类数据库查询：客户查询支持门店、业务阶段、状态、建档日期、姓名／生日、直接跳页，以及二次确认后的封存／恢复；充值／核销查询支持门店、项目、类型、状态、日期、姓名／生日、直接跳页与精确工单详情，完整工单号保持单行且不截断，宽度不足由表格容器内部横向滑动承担。核销查询不再显示不适用于正常／体验核销的“待审核／已通过／已驳回”汇总卡，只在结果卡显示总条数；充值／退费仍保留审核汇总。门店查询由服务端锁定本店，总部可以选择全部或具体门店；老师不进入通用客户查询页，只从本人工作台的业务明细和有权限的客户列表进入客户主页。
5. 客户主页、客户历史工单链接和四类数据库工单详情；核销详情固定五个照片位，支持单图重读、高清原图预览、相册保存及原提交账号在服务端时限内补充照片。正常核销与体验核销不显示不存在的审核时间，只有历史补录核销保留审核记录。四类工单均使用与产品样例相同的单据排版导出 PDF／图片，PDF 按标准 A4 真正分页。
