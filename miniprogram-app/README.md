...
5. 客户主页、客户历史工单链接和四类数据库工单详情；客户主页只展示并预览客户照片，不提供重读或相册保存，项目和业务老师只显示名称／姓名，留言发送按钮在输入框下方完整居中。核销详情固定五个照片位，支持单图重读、高清原图预览、相册保存及原提交账号在服务端时限内补充照片。正常核销与体验核销不显示不存在的审核时间，只有历史补录核销保留审核记录。四类工单均使用与产品样例相同的单据排版导出 PDF／图片，PDF 按标准 A4 真正分页。
...
> 交付边界：当前仓库代码与文档按 `staffAccount v69`、`faceRecognition v90`、`verificationPhoto v9`、`teacherCreate v6` 编排。CloudBase 小程序授权已经成功；手机号＋密码已在开发者工具使用既有账号进入总部主页并完成 `staffAccount.session` 回读。小程序 `0.2.0` 已于 2026-08-25 上传为体验版，但尚未提交微信审核或正式发布。`WX_MICRO_APP` 身份源、微信手机号能力与计费仍未开通验收，当前 AppID 的正式 `request` 合法域名和真机验收也未完成；线上公众仍不能视为已支持微信手机号快捷登录。本轮没有修改或上传云函数，也没有运行 SQL 或发布网站。
...
- 2026-08-25 上传的体验版 `0.2.0` 总包为 `1,583,061` 字节，主包为 `1,360,981` 字节，均低于微信限制。启动页和共用 WXSS 页面留在主包，其余 12 个业务页使用原目录普通分包；分包不得改成 `independent`，否则会失去主包的会话、服务、组件和全局样式。上传配置会过滤不可达 npm 产物、sourcemap、类型声明和本地依赖元数据，测试会保留至少 200KB 的主包预算余量。
