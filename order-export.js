(() => {
  ...
  function drawDocumentHeader(context, documentData, productLogo, draw) {
    ...
    context.fillText(kind, PAGE_MARGIN + 19, top + 23);
  ...
  function drawInfoGrid(context, items, y, draw, paginate, columns = 2) {
    ...
    for (let index = 0; index < items.length;) {
      ...
      const requestedSpan = /时间$/.test(text(firstItem?.label, ""))
        ? columnCount
        : Math.max(1, Math.min(columnCount, Number(firstItem?.span || 1)));
      ...
      while (index < items.length && row.length < columnCount) {
        const item = items[index];
        if (/时间$/.test(text(item?.label, "")) || Number(item?.span || 1) >= columnCount) break;
        row.push(item);
        index += 1;
      }
