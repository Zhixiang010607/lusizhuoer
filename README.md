...
运营管理、运营创建页和运营首页均已移除。迁移 `047_retire_operation_accounts.sql` 保留旧审核外键并封存运营身份。老师账号由 `teacherCreate v6` 只按姓名、手机号和密码创建，不采集或维护老师人脸。`staffAccount v75` 与 `faceRecognition v96` 已物理删除旧 Saga、老师人脸委托动作和兼容入口。迁移 053 删除旧 `teacher_face_operations` 表与六个私有函数，不删除老师、额度、工单或历史业务。
